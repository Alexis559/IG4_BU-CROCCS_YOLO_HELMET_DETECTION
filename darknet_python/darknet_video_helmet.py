...
def cvDrawBoxes(detections, img):
    for detection in detections:
        ...
        if detection.label == 'person':
            # If the person is related with a helmet and a motorbike then it's good we display a green bounding boxe
            if len(detection.objects_related) == 2:
                cv2.rectangle(img, pt1, pt2, (0, 255, 0), 2)
                color = [0, 255, 0]

            # If the person is related just with a helmet or a motorbike then it's not good we display a red bounding boxe
...
if __name__ == "__main__":
    ...
    a.add_argument("--cfg_file", help="path to cfg file")
    a.add_argument("--weights_file", help="path to weights file")
    a.add_argument("--data_file", help="path to data file")
    a.add_argument("--video_file", help="path to the video file")
    a.add_argument("--threshold", help="threshold for detection")
    args = a.parse_args()
